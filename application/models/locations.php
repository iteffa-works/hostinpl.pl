...
class locationsModel extends Model {
	...
	public function createLocationStats($data) {
		$sql = "INSERT INTO `locations_stats` SET ";
		$sql .= "location_id = '" . (int)$data['location_id'] . "', ";
		$sql .= "location_stats_date = NOW(), ";
		$sql .= "location_stats_cpu = '" . (int)$data['location_stats_cpu'] . "', ";	
		$sql .= "location_stats_ram = '" . (int)$data['location_stats_ram'] . "', ";		
		$sql .= "location_stats_hdd = '" . (int)$data['location_stats_hdd'] . "'";				
		$this->db->query($sql);
		return $this->db->getLastId();
	}

	public function clearLocationStats() {
		$sql = "DELETE FROM `locations_stats` WHERE location_stats_date < NOW() - INTERVAL 7 DAY";
		$this->db->query($sql);
	}
	
	public function getLocationStats($locationid, $start, $end) {
		$sql = "SELECT * FROM `locations_stats` WHERE location_id = '" . (int)$locationid . "' AND location_stats_date BETWEEN " . $start . " AND " . $end . " ORDER BY location_stats_date";
		$query = $this->db->query($sql);
		return $query->rows;
	}
}
